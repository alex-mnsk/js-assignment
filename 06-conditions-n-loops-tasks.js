...
function findFirstSingleChar(str) {
    return str
            .split('')
            .filter( (val) => str.indexOf(val) === str.lastIndexOf(val))[0];
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
    ...
    if (a > b) { dig2 = a, dig1 = b }
    if (isStartIncluded === true) { bracket1 = '[' }
    if (isEndIncluded === true) { bracket2 = ']' }
    
    return `${bracket1}${dig1}, ${dig2}${bracket2}`;
}
...
function getDigitalRoot(num) {
    ...
    return number < 9 ? number : getDigitalRoot(number);     
}
...
function timespanToHumanString(startDate, endDate) {
        ...
        if (time <= 45 * minute) return `${Math.round((time - 1) / minute)} minutes ago`;
        if (time <= 90 * minute) return 'an hour ago';
