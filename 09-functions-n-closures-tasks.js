...
function getPolynom(...args) {
    return (x) =>   
        args.reduce((prev, curr, i) => 
            prev + Math.pow(x, args.length - i - 1 ) * curr, 0); 
}
...
function logger(func, logFunc) {
    return function (...args) {
        let argmnts = JSON.stringify(args).slice(1,-1);      
        
        logFunc( `${func.name}(${argmnts}) starts` );
        
        let result = func.apply(this,args);        
        logFunc( `${func.name}(${argmnts}) ends` );
...
function partialUsingArguments(fn, ...first) {
    return (...others) => {
        return fn.apply(null, first.concat(others));
    }
}
